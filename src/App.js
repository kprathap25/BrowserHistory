...
import ImageThumbnail from './Components/Components/ImageThumbnail/index'

import MatchGame from './Components/Components/MatchGame/index'
import './App.css'
...
class App extends Component {
  state = {
    ...
    isClickedId: imagesList[0].category,
    resultDisplay: false,
  }
  ...
  onChangeScore = id => {
    ...
    if (id === imageId) {
      ...
      this.setState(prevState => ({
        ...
        displayImage: imagesList[newIndex].imageUrl,
        imageId: imagesList[newIndex].id,
      }))
    } else {
  ...
  onChangeTabStyle = tabId => {
    this.setState({isClickedId: tabId})
  }
  ...
  render() {
    ...
    const finalList = imagesList.filter(
      eachList => eachList.category === isClickedId,
    )
      ...
      <div className="bg-container">
        <nav className="nav-container">
          <img
            ...
            className="logo"
          />
          <ul className="score-seconds-container">
            <li>
        ...
        {resultDisplay ? (
          <div className="result-container">
            <img
              ...
              className="trophy"
            />
            <p className="result-para">YOUR SCORE</p>
            <p className="result-para-score">{score}</p>
            <button
              className="btn-again"
              type="button"
            ...
            >
              <img
                ...
                alt="reset"
              />
              <p className="result-para">PLAY AGAIN</p>
            </button>
          </div>
        ) : (
          <div className="sub-container">
            <img className="img-main" src={displayImage} alt="match" />
            <ul className="thumbnail-list-container">
              {tabsList.map(eachThumbnail => (
                <ImageThumbnail
                  key={eachThumbnail.tabId}
                  listOfThumbnail={eachThumbnail}
                  isTabClicked={isClickedId === eachThumbnail.tabId}
                  onChangeTabStyle={this.onChangeTabStyle}
                />
              ))}
            </ul>
            <ul className="image-list-container">
              {finalList.map(eachImageList => (
                <MatchGame
                  listOfImageDetails={eachImageList}
